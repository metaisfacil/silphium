...
ifeq ($(OS),Windows_NT)
icon:
	$(PYTHON_ICON) scripts/generate_windows_icon.py --svg frontend/public/silphium.svg --app-png build/appicon.png --ico build/windows/icon.ico
...
icon:
	...
	sips -s format png frontend/public/silphium.svg --out build/appicon.source.png >/dev/null
